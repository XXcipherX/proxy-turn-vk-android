...
По умолчанию включён режим **«Авто капча»**. Оркестратор делает до двух попыток Go v2-решателя, затем одну попытку Auto WebView продолжительностью до 20 секунд, затем финальную Go-попытку. Если всё это не помогло, приложение открывает ручной WebView. Автоматический и ручной WebView используют реальный профиль установленного Android System WebView без подмены User-Agent. Авто-режим можно отключить, тогда в UI появляются ручные настройки метода и режима прохождения.
